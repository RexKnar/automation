...
export class MetaService {
    ...
    async checkInstagramFollow(userId: string, pageId: string, accessToken: string): Promise<boolean> {
        console.log(`[MetaService] Checking if user ${userId} follows page ${pageId}`);

        try {
            // Using v21.0 as per project standard, but user mentioned v24.0. 
            // The field `is_user_follow_business` is available on the User node.
            // We use the Page Access Token.
            const url = `https://graph.instagram.com/v21.0/${userId}?fields=is_user_follow_business`;
            ...
            console.log(`[MetaService] Follow Check Response for ${userId}:`, JSON.stringify(data));
            ...
            console.error(`[MetaService] Failed to check follow status for ${userId}:`, error?.response?.data || error.message);
            return false;
        }
    }
